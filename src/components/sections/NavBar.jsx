...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white text-[#1F2937] px-4 sm:px-8 py-4 flex items-center justify-between">
      {/* Logo */}
      <div className="text-lg sm:text-xl font-bold tracking-tight text-primary">
      ...
      <ul className="hidden sm:flex space-x-6 text-sm font-medium">
        ...
        <li><a href="/explore" className="hover:text-[#F3C623] transition">Explore</a></li>
        <li><a href="/how-it-works" className="hover:text-[#F3C623] transition">How It Works</a></li>
      </ul>

      {/* Desktop Button */}
      <div className="hidden sm:block">
        <button className="bg-primary bg-primary-hover text-white text-sm font-semibold px-4 py-2 rounded-md transition">
          Connect Wallet
        </button>
      </div>
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-md z-50 sm:hidden">
          <ul className="flex flex-col px-6 py-4 space-y-3 text-sm font-medium">
            ...
            <li><a href="/explore" className="hover:text-[#F3C623] transition" onClick={() => setMenuOpen(false)}>Explore</a></li>
